...
(function (definition) {
	...
	/**
	 * Sets the [lookup driver]{@link CIQ.UI.Lookup} to be used with the [cq-lookup web component]{@link WebComponents.cq-lookup}
	 *
	 * The lookup driver will be called to search for matching symols as data is typed into the set `UIContext.UISymbolLookup` field.
	 *
	...
	BaseComponent.getHelper=function(node, binding, attribute){
		...
		if(paren==-1 || beforeParen.indexOf(".")!=-1){ // Layout or Layout.Chart or Layout.Chart('blah')
			...
			helper=node.context.getAdvertised(helperName);
	...
	/**
	 * Used to define a symbol search-engine lookup driver to be used with the [cq-lookup web component]{@link WebComponents.cq-lookup}
	 *
	 * The lookup driver will be called to search for matching symols as data is typed into the set `UIContext.UISymbolLookup` field.
	 *
	 * See `function startUI()` in sample-template-advanced.html for complete sample implementation.
	 ...
	 */
	CIQ.UI.Lookup.Driver=function(exchanges){};
