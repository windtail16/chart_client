;(function(root, factory) {
	...
	Studies.prototype.renderMenu=function(){
		...
		for(var field in CIQ.Studies.studyLibrary){
			...
			if(!sd || this.excludedStudies[field] || this.excludedStudies[sd.name]) continue;
			if(!sd.name) sd.name=field; // Make sure there's always a name
			if(sd.name == 'Volume Chart') {
				sd.name = '거래량';
			}else if(sd.name == 'Momentum Indicator') {
				sd.name = '모멘텀';
			}else if(sd.name == 'Bollinger Bands') {
				sd.name = '볼린저 밴드';
			}else if(sd.name == 'Bollinger Bandwidth') {
				sd.name = '볼린저 Bandwidth';
			}else if(sd.name == 'Disparity Index') {
				sd.name = '이격도';
			}else if(sd.name == 'Moving Average') {
				sd.name = '이동평균선';
			}else if(sd.name == 'Ichimoku Clouds') {
				sd.name = '일목균형표';
			}else if(sd.name == 'Stochastics') {
				sd.name = '스토캐스틱';
			}else if(sd.name == 'Stochastic Momentum Index') {
				sd.name = '스토캐스틱 모멘텀';
			}else if(sd.name == 'Psychological Line') {
				sd.name = '심리도';
			}else if(sd.name == 'Standard Deviation') {
				sd.name = '표준편차';
			}
		...
		for(var i=0;i<alphabetized.length;i++){
			...
			menuItem.selectFC=tapFn(alphabetized[i], this.context);
...
<cq-dialog cq-timezone-dialog>
	...
	Dialog.prototype.show=function(params){
		...
		if(!this.uiManager.overlay && !params.bypassOverlay){
			...
			context.node.append(this.uiManager.overlay);
		}
...
<cq-lookup cq-keystroke-claim cq-keystroke-default>
	...
	/**
	 * Processes an array of results returned by the {@link CIQ.UI.Lookup.Driver} and displays them.
	 * 
